# geofmigliacci

[![CI](https://github.com/geofmigliacci/geofmigliacci/actions/workflows/ci.yml/badge.svg)](https://github.com/geofmigliacci/geofmigliacci/actions/workflows/ci.yml)
[![codecov](https://codecov.io/gh/geofmigliacci/geofmigliacci/graph/badge.svg)](https://codecov.io/gh/geofmigliacci/geofmigliacci)
[![geofmigliacci.dev](https://img.shields.io/website?url=https%3A%2F%2Fwww.geofmigliacci.dev&label=geofmigliacci.dev&up_color=brightgreen&down_color=red)](https://www.geofmigliacci.dev)

My personal site — I'm a software engineer who cares about backend systems, performance, and domain-driven design, and this is where I write about it. Built with Next.js 16, TypeScript, and Tailwind CSS v4, with an MDX-based article system, full SEO/PWA setup, and a CI/CD pipeline (lint, typecheck, tests, coverage) gating every deploy to Vercel.
